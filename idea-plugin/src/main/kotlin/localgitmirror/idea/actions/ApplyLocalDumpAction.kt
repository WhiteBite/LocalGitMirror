...
class ApplyLocalDumpAction : AnAction() {
  override fun actionPerformed(e: AnActionEvent) {
    ...
    descriptor.title = "Select encrypted dump (.dmp)"
    descriptor.description = "Pick dump_*.dmp generated by LocalGitMirror"
    descriptor.withFileFilter { vf -> vf.extension?.equals("dmp", ignoreCase = true) == true }
    ...
    if (!dumpFile.exists() || !dumpFile.isFile) {
      notify(project, "Selected dump file does not exist", NotificationType.ERROR)
      return
    }

    val mode = Messages.showEditableChooseDialog(
      "Apply mode",
      "LocalGitMirror: Apply local dump",
      null,
    ...
    val branchName: String? = if (mode == "new-branch") {
      val defaultName = "local-dump-${System.currentTimeMillis()}"
      Messages.showInputDialog(project, "New branch name", "LocalGitMirror: Apply local dump", null, defaultName, null)?.trim()
    } else null
    if (mode == "new-branch" && branchName.isNullOrBlank()) return

    ProgressManager.getInstance().run(object : Task.Backgroundable(project, "LocalGitMirror: Apply local dump", false) {
      override fun run(indicator: ProgressIndicator) {
        indicator.text = "Preparing local apply"
        indicator.text = "Applying local dump"
        ...
        notify(project, "Local dump applied successfully", NotificationType.INFORMATION)
      }
    })
  }
