  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    ProgressManager.getInstance().run(object : Task.Backgroundable(project, "LocalGitMirror: Preflight", false) {
      override fun run(indicator: ProgressIndicator) {
        ...
        val summary = if (report.ok) "Preflight OK" else "Preflight has issues"
        notify(project, "$summary (target=${report.targetRepo ?: "(none)"})", if (report.ok) NotificationType.INFORMATION else NotificationType.WARNING)
      }
    })
  }
