"""Python-only stealth dump encryption/decryption helpers (no 7-Zip required).

Format (versioned custom container, binary):
  magic(8) = b'LGMSTRL1'
  salt(16)
  nonce(12)
...
def decrypt_dump_to_bundle(dump_path: Path, out_bundle_path: Path, password: str) -> None:
    ...
    min_len = len(MAGIC) + SALT_SIZE + NONCE_SIZE + 8 + 16  # GCM tag min overhead in ciphertext
    if len(raw) < min_len:
        raise ValueError("Dump file too small")
    ...
    if len(ciphertext) != ciphertext_len:
        raise ValueError("Corrupted dump payload")
